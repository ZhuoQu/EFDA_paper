  ...
  for (varnm in 1:nbvar) {
    ...
    plot(x, y[, varnm], lty = 1, lwd = 2, col = 1, type = "n", 
       ...
       main = titles[varnm])
        ...
        plot(x, y, lty = 1, lwd = 2, col = 1, type = "n", 
             xlim = xlim, 
             ylim = range(lapply(param_PD, function(k) {k$y})),
             xlab = xlab, 
             ylab = ylabs, 
             main = titles) 
    }
    ...
    if (param == 1) {
      lapply(outlier_info, function(k) {
        ...
        lines(k$argvals, k$y[, varnm], lwd = 2, lty = 2, col = "red")
              ...
              lines(k$argvals, k$y, lwd = 2, col = "grey", type = "l")
              lines(k$argvals, k$y, lwd = 2, lty = 2, col = "red") 
          }
      })
      ...
      lapply(outlier_candidate, function(k) {
        ...
        lines(param_PD[[k]]$argvals, 
              ...
              lines(param_PD[[k]]$argvals, 
                    param_PD[[k]]$y[1:length(param_PD[[k]]$argvals)], 
                    lwd = 2, col = "grey", type = "l")
              lines(param_PD[[k]]$argvals, 
                    param_PD[[k]]$y[1:length(param_PD[[k]]$argvals)], 
                    lwd = 2, lty = 2, col = "green")     
      ...
      lapply(outlier_info, function(k) {
        ...
        lines(k$time_percentage, k$y[, varnm], lty = 2, lwd = 2, col = "red")
              ...
              lines(k$time_percentage, k$y, lwd = 2, col = "grey", type = "l")
              lines(k$time_percentage, k$y, lty = 2, lwd = 2, col = "red")  
          }
      })
      lapply(outlier_candidate, function(k) {
          if (nbvar > 1) {
        ...
        lines(param_PD[[k]]$time_percentage, 
              ...
              lines(param_PD[[k]]$time_percentage, 
                    param_PD[[k]]$y[1:length(param_PD[[k]]$time_percentage)], 
                    lwd = 2, col = "grey", type = "l")
              lines(param_PD[[k]]$time_percentage, 
                    param_PD[[k]]$y[1:length(param_PD[[k]]$time_percentage)], 
                    lwd = 2, lty = 2, col = "green")
    ...
    lines(x[-length(x)], y[-length(x), varnm], lty = 2, lwd = 2)
        ...
        lines(x[-length(x)], y[-length(x)], lwd = 2, col = "white", type = "l")
        lines(x[-length(x)], y[-length(x)], lty = 2, lwd = 2)
    }
