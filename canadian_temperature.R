...
indigo = rgb(75, 0, 130, maxColorValue = 255)
color_palette <- c("red", "orange", "yellow", "green", "blue", indigo, "violet")
...
order_temp_decrease <- order(extremed_temp, decreasing = TRUE)
color_palette <- c("red", "orange", "yellow", "green", "cyan", "blue", "purple")
CanadianWeather$coordinates
pdf(file = "canada_weather.pdf", width = 8, height = 4)
# plot(CanadianWeather$coordinates[, 2], CanadianWeather$coordinates[, 1], type = "n")
# lapply(1:length(order_temp_decrease), function(l) {
#     points(CanadianWeather$coordinates[order_temp_decrease[l], 2], 
#            CanadianWeather$coordinates[order_temp_decrease[l], 1],
#            col = color_palette[ceiling(l / 5)], pch = 19)
#     text(CanadianWeather$coordinates[order_temp_decrease[l], 2], 
#          CanadianWeather$coordinates[order_temp_decrease[l], 1], 
#          labels = order_temp_decrease[l])
# })
...
plot(1:12, CanadianWeather$monthlyTemp[, 1], type = "n", 
     ylim = range(CanadianWeather$monthlyTemp, na.rm = TRUE), ylab ="Temperature (\u00B0C)",
     xlab = "Months", main = "(c) Monthly Temperature Curves in Canada", cex.main = 1)
lapply(1:length(order_temp_decrease), function(k) {
    lines(1:12, CanadianWeather$monthlyTemp[, order_temp_decrease[k]], 
          col = color_palette[ceiling(k / 5)])
    # text(12, CanadianWeather$monthlyTemp[12, order_temp_decrease[k]], 
    #      labels = order_temp_decrease[k])
})
# lapply(1:length(order_decrease), function(k) {
#   points(1:12, CanadianWeather$monthlyTemp[, order_decrease[k]], 
#          cex = 0.7)
...
plot(1:12, CanadianWeather$monthlyPrecip[, 1], type = "n", 
     ...
     xlab = "Months", main = "(c) Monthly Precipitation Curves in Canada", cex.main = 1)
lapply(1:length(order_temp_decrease), function(k) {
    lines(1:12, CanadianWeather$monthlyPrecip[, order_temp_decrease[k]],
          col = color_palette[ceiling(k / 5)]
    )
    # text(12, CanadianWeather$monthlyPrecip[12, order_temp_decrease[k]], 
    #      labels = order_temp_decrease[k])
})
dev.off()
